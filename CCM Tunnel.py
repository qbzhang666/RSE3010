...
def grc_carranza_torres_hb(pi, p0, a, nu, Em, sigma_ci, mb, s, a_HB):
    ...
    if pi >= pcr:
        ...
        # Use pi_eff consistently in the plastic-radius expression.
        # Using the raw pi in the denominator while evaluating k_tang at pi_eff
        # caused the GRC to curl/back-bend close to zero support pressure.
        Rp = a * ((2 * (p0 * (k_tang - 1) + sigma_cm_tang)) /
                  ((1 + k_tang) * (pi_eff * (k_tang - 1) + sigma_cm_tang))) ** (1 / (k_tang - 1))
